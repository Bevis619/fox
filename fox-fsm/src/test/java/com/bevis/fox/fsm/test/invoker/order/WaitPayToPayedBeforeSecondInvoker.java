 ...
 */
@Service
@StateConfig(biz = "ORDER", from = "HNADING", to = "CANCELED", invokeType = InvokeTypeEnum.BEFORE, order = 1)
public class WaitPayToPayedBeforeSecondInvoker implements StateInvoker<OrderVO, BizTypeEnum, OrderStateEnum> {
    /**
     * Invoke boolean.
     *
    ...
    public boolean invoke(StateContext<OrderVO, BizTypeEnum, OrderStateEnum> context) {
        System.out.println("Before second-" + context.getData().getName() + ":" + context.toString());
        return true;
    }
}
